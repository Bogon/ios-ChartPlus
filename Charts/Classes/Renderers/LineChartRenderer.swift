...
public class LineChartRenderer: LineScatterCandleRadarChartRenderer
{
    ...
    /**
     ...
     - parameter context: 图像上下文对象
     - parameter dataSet: 数据点集合
     */
            ...
            switch dataSet.drawLinesStyle
            {
                ...
                case .SolidAndDottedLine:
                    ...
                    *设置根据
                    空格点，绘制虚线
                    */
                    if dataSet.breakpointValue != 0
                    {
                        drawBreakPointLinear(context: context, dataSet: dataSet, entries: entries)
                    }
                    break
            }
        }
    ...
    private var _lineSegmentsForRealLine = [CGPoint](count: 2, repeatedValue: CGPoint())
    /**
     *绘制虚线，根据已知的空格点，进行绘制虚线的准备工作
     *步骤：1、过滤相关数据，重新设定绘制的点，以及起始位置
     - parameter context: 图像上下文对象
     - parameter dataSet: 数据对象模型数组
     - parameter entries: 单个对象实体对象
     */
    internal func drawBreakPointLinear(context context: CGContext, dataSet: LineChartDataSet, entries: [ChartDataEntry])
    {
        ...
        */
        ...
        {
            if (_lineSegmentsForRealLine.count != 2)
            {
                _lineSegmentsForRealLine = [CGPoint](count: 2, repeatedValue: CGPoint())
            }
            ...
            for (var j = minx, count = Int(ceil(CGFloat(maxx - minx) * phaseX + CGFloat(minx))); j < count; j++)
            {
                ...
                _lineSegmentsForRealLine[0].x = CGFloat(e.xIndex)
                _lineSegmentsForRealLine[0].y = CGFloat(e.value) * phaseY
                _lineSegmentsForRealLine[0] = CGPointApplyAffineTransform(_lineSegmentsForRealLine[0], valueToPixelMatrix)
                if (j + 1 < count)
                {
                    e = entries[j + 1]
                    
                    _lineSegmentsForRealLine[1].x = CGFloat(e.xIndex)
                    _lineSegmentsForRealLine[1].y = CGFloat(e.value) * phaseY
                    _lineSegmentsForRealLine[1] = CGPointApplyAffineTransform(_lineSegmentsForRealLine[1], valueToPixelMatrix)
                }
                else
                {
                    _lineSegmentsForRealLine[1] = _lineSegmentsForRealLine[0]
                }
                
                if (!viewPortHandler.isInBoundsRight(_lineSegmentsForRealLine[0].x))
                {
                    break
                }
                
                // make sure the lines don't do shitty things outside bounds
                if (!viewPortHandler.isInBoundsLeft(_lineSegmentsForRealLine[1].x)
                    || (!viewPortHandler.isInBoundsTop(_lineSegments[0].y) && !viewPortHandler.isInBoundsBottom(_lineSegmentsForRealLine[1].y))
                    || (!viewPortHandler.isInBoundsTop(_lineSegments[0].y) && !viewPortHandler.isInBoundsBottom(_lineSegmentsForRealLine[1].y)))
                {
                    continue
                }
                ...
                CGContextStrokeLineSegments(context, _lineSegmentsForRealLine, 2)
            }
        }
        else
        { // only one color per dataset
            ...
            var e3: ChartDataEntry!
            
            
            if (_lineSegmentsForRealLine.count != max((entries.count - 1) * 2, 2))
            {
                _lineSegmentsForRealLine = [CGPoint](count: max((entries.count - 1) * 2, 2), repeatedValue: CGPoint())
            }
            ...
            */
            ...
            for (var x = count > 1 ? minx + 1 : minx, j = 0; x < count; x++)
            {
                ...
                e2 = entries[x]
                e3 = entries[x == (count-1) ? 0 : (x + 1)]
                
                if e2.value == 110 && e1.value != 110 && e3.value != 110{
                    
                        _lineSegmentsForRealLine[j++] = CGPointApplyAffineTransform(CGPoint(x: CGFloat(e1.xIndex), y: CGFloat(e1.value) * phaseY), valueToPixelMatrix)
                        _lineSegmentsForRealLine[j++] = CGPointApplyAffineTransform(CGPoint(x: CGFloat(e3.xIndex), y: CGFloat(e3.value) * phaseY), valueToPixelMatrix)
                }
            ...
            CGContextSetLineDash(context, 0, lineArray, lineArray.count)
            
            //绘制实线
            let size = max((count - minx - 1) * 2, 2)
            CGContextSetStrokeColorWithColor(context, dataSet.colorAt(1).CGColor)
            CGContextStrokeLineSegments(context, _lineSegmentsForRealLine, size)
        ...
        */
        ...
        { // only one color per dataset
            ...
            for (var x = count > 1 ? minx + 1 : minx, j = 0; x < count; x++)
            {
                ...
                let drawBreakpointValue:Double = dataSet.breakpointValue
                switch dataSet.drawLinesStyle
                {
                case .NormalStraightLine:
                    ...
                    break
                case .BreakpointLine:
                    if e1.value != drawBreakpointValue && e2.value != drawBreakpointValue
                    {
                        _lineSegments[j++] = CGPointApplyAffineTransform(CGPoint(x: CGFloat(e1.xIndex), y: CGFloat(e1.value) * phaseY), valueToPixelMatrix)
                        _lineSegments[j++] = CGPointApplyAffineTransform(CGPoint(x: CGFloat(e2.xIndex), y: CGFloat(e2.value) * phaseY), valueToPixelMatrix)
